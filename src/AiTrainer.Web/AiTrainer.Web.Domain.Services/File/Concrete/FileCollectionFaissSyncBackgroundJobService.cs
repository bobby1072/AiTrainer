...
internal class FileCollectionFaissSyncBackgroundJobService : BackgroundService
{
    ...
    public FileCollectionFaissSyncBackgroundJobService(
        ILogger<FileCollectionFaissSyncBackgroundJobService> logger,
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogDebug(
                    "--------Processing faiss sync request in {BackgroundServiceName} for collectionId {CollectionId} and userId {UserId}--------",
                    nameof(FileCollectionFaissSyncBackgroundJobService),
                    job.CollectionId,
                    job.User.Id
                );
            ...
            _logger.LogDebug(
                "--------Processing finished for faiss sync request for collectionId {CollectionId} and userId {UserId}--------",
                job.CollectionId,
                job.User.Id
            );
        }

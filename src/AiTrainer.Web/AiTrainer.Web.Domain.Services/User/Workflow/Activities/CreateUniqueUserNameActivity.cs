...
namespace AiTrainer.Web.Domain.Services.User.Workflow.Activities
{
    ...
    {
        public override string Description => "This activity is used to create a unique username for a brand new user";
        ...
        public CreateUniqueUserNameActivity(IRepository<UserEntity, Guid, Models.User> repo, ILogger<CreateUniqueUserNameActivity> logger)
        {
            _repo = repo;
            _logger = logger;
        }

        public override async Task<(ActivityResultEnum ActivityResult, (Models.User?, string?) ActualResult)> ExecuteAsync(Models.User? workflowContextItem)
        {
            try
            {
                ...
                if (workflowContextItem?.Id is null && workflowContextItem?.Username is null)
                {
                    ...
                    var foundUserOfSameUsername = await _repo.GetOne(newUsername, "Username".ToPascalCase());
                    ...
                    return (ActivityResultEnum.Success, (workflowContextItem, $"{usernamePrefix}{Guid.NewGuid().ToString().Replace("-", "")}"));

                }

                return (ActivityResultEnum.Success, (workflowContextItem, null));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Exception occurred while creating a unique username for a new user with message {Message}", ex.Message);

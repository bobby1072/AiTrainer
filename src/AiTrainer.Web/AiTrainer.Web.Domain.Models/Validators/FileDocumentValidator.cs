    ...
    internal class FileDocumentValidator : BaseValidator<FileDocument>
    {
        public FileDocumentValidator()
        {
            RuleFor(x => x.FileType)
                .Must(NotNullFileType)
                .WithMessage(ValidatorConstants.InvalidFileType);

            RuleFor(x => x.FileName)
                .Must(IsStringNotNullEmptyOrWhitespace)
                .WithMessage(ValidatorConstants.InvalidFileName);

            RuleFor(x => x.FileName)
                .Must(x => NotLongerThan(x, 150))
                .WithMessage(ValidatorConstants.InvalidFileName);
            ...
            RuleFor(x => x.FileData)
                .Must(FileIsNotTooLarge)
                .WithMessage(ValidatorConstants.InvalidFileData);
        ...
        private static bool NotNullFileType(FileTypeEnum fileType) =>
            fileType == FileTypeEnum.Null ? false : true;

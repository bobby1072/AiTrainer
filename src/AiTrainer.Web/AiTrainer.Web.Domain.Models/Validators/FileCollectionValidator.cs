...
namespace AiTrainer.Web.Domain.Models.Validators
{
    internal class FileCollectionValidator : BaseValidator<FileCollection>
    {
        public FileCollectionValidator()
        {
            ...
            RuleFor(x => x.CollectionName).Must(IsStringNotNullEmptyOrWhitespace).WithMessage(ValidatorConstants.InvalidName);
            RuleFor(x => x.CollectionName).Must(x => NotLongerThan(x, 100)).WithMessage(ValidatorConstants.InvalidName);

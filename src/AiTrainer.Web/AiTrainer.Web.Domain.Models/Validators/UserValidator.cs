...
namespace AiTrainer.Web.Domain.Models.Validators
{
    internal class UserValidator : BaseValidator<User>
    {
        public UserValidator()
        {
            ...
            RuleFor(x => x.Username).Must(x => NotLongerThan(x, 50)).WithMessage(ValidatorConstants.InvalidName);
            RuleFor(x => x.Name).Must(x => NotLongerThan(x, 50)).WithMessage(ValidatorConstants.InvalidName);
